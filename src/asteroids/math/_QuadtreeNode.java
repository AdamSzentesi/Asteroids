...
public class _QuadtreeNode
{
	private final int level;
	private final int MAX_OBJECTS;
	private final int MAX_LEVEL;
	private _QuadtreeNode[] nodes;
	...
	public _QuadtreeNode(int level, int MAX_OBJECTS, int MAX_LEVEL, QuadtreeBounds bounds)
	{
//		System.out.println(this.getClass().getSimpleName() + ": created on level " + level);
		this.level = level;
		this.MAX_OBJECTS = MAX_OBJECTS;
		this.MAX_LEVEL = MAX_LEVEL;
		this.nodes = new _QuadtreeNode[4];
...
//		System.out.println(this.getClass().getSimpleName() + ": split on level " + this.level);
		...
		nodes[0] = new _QuadtreeNode(this.level + 1, MAX_OBJECTS, MAX_LEVEL, new QuadtreeBounds(this.bounds.min.add(new Vector2f(0, halfHeight)), halfWidth, halfHeight));
		nodes[1] = new _QuadtreeNode(this.level + 1, MAX_OBJECTS, MAX_LEVEL, new QuadtreeBounds(this.bounds.min.add(new Vector2f(halfWidth, halfHeight)), halfWidth, halfHeight));
		nodes[2] = new _QuadtreeNode(this.level + 1, MAX_OBJECTS, MAX_LEVEL, new QuadtreeBounds(this.bounds.min.add(new Vector2f(halfWidth, 0)), halfWidth, halfHeight));
		nodes[3] = new _QuadtreeNode(this.level + 1, MAX_OBJECTS, MAX_LEVEL, new QuadtreeBounds(this.bounds.min, halfWidth, halfHeight));
	}
...
//		System.out.println(this.level + " " + this.occupied);
	...
	public List<Physics2DAABB> getOccupiedList()
	{
		...
		if(this.nodes[0] != null)
		{
			for(_QuadtreeNode node : this.nodes)
			{
